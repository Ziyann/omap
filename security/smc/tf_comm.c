...
static bool sigkill_pending(struct tf_connection *connection)
{
	if (connection && connection->owner == TF_CONNECTION_OWNER_KERNEL)
		return false;
	...
	u32 flags, struct mm_struct *mm, struct page *page)
{
	...
	descriptor = tf_get_l3_descriptor_common_lpae(page_vaddr, mm);
	if (descriptor == 0) {
		*l3_page_descriptor = L3_DESCRIPTOR_FAULT;
		return;
	}
		...
		signed long *relative_timeout_jiffies)
{
	...
	*relative_timeout_jiffies = 0;
	...
	if (timeout == TIME_INFINITE) {
		dprintk(KERN_DEBUG "tf_test_s_timeout: timeout is infinite\n");
...
static void tf_copy_answers(struct tf_comm *comm)
{
	...
	if (test_bit(TF_COMM_FLAG_L1_SHARED_ALLOCATED, &(comm->flags))) {
		...
		while (first_answer != first_free_answer) {
			...
			for (i = 0; i < sizeof(struct tf_answer_header)/sizeof(u32); i++)
				temp[i] = comm->l1_buffer->answer_queue[
					(first_answer + i) % TF_S_ANSWER_QUEUE_CAPACITY];
...
copy_answers:
	...
	if (command_status == TF_COMMAND_STATE_ABORTED) {
		...
		dprintk(KERN_ERR "[pid=%d] tf_send_recv: "
			"Command status is ABORTED. Exit with 0x%x\n",
			current->pid, result);
		goto exit;
	}
	 ...
	 */
	if (bKillable && (sigkill_pending(connection))) {
		if (command_status == TF_COMMAND_STATE_PENDING)
...
wait:
	...
	if (schedule_timeout(nRelativeTimeoutJiffies) == 0)
		dprintk(KERN_INFO "tf_send_recv: timeout expired\n");
	else
		dprintk(KERN_INFO "tf_send_recv: signal delivered\n");
