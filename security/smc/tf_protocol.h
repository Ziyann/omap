...
struct tf_command_param_temp_memref_lpae {
	u64    descriptor; /* data pointer for exchange message.*/
	u32    size;
	u32    offset;
};

union tf_command_param_lpae {
 ...
 * protocol spec. */
#define TF_MAX_COARSE_PAGES_8DESC                 8
...
/* TF_MAX_COARSE_PAGES_LPAE is the number of level 1 descriptors (describing
 * 2MB each) that can be shared with the secure world in a single registered
 * shared memory block. It must be kept in synch with
 * SCHANNEL6_MAX_DESCRIPTORS_PER_REGISTERED_SHARED_MEM in the SChannel
 * protocol spec. */
#define TF_MAX_COARSE_PAGES_LPAE                     64
#define TF_DESCRIPTOR_TABLE_CAPACITY_BIT_SHIFT_LPAE  (9)
#define TF_DESCRIPTOR_TABLE_CAPACITY_LPAE \
	(1 << TF_DESCRIPTOR_TABLE_CAPACITY_BIT_SHIFT_LPAE)
#define TF_DESCRIPTOR_TABLE_CAPACITY_MASK_LPAE \
	(TF_DESCRIPTOR_TABLE_CAPACITY_LPAE - 1)

/* Shared memories coarse pages can map up to 2MB */
#define TF_MAX_COARSE_PAGE_MAPPED_SIZE_LPAE \
	(PAGE_SIZE * TF_DESCRIPTOR_TABLE_CAPACITY_LPAE)
 ...
 */
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
struct tf_command_open_client_session_lpae {
	u8                            message_size;
	u8                            message_type;
	u16                           param_types;
	/* an opaque Normal World identifier for the operation */
	...
	u64                           timeout;
	struct tf_uuid                destination_uuid;
	union tf_command_param_lpae   params[4];
	u32                           login_type;
	/*
	 * Size = 0 for public, [16] for group identification, [20] for
	 * authentication
 ...
 */
struct tf_command_register_shared_memory_128 {
	u8  message_size;
	u8  message_type;
	u16 memory_flags;
	u32 operation_id;
...
struct tf_command_register_shared_memory_8 {
	u8  message_size;
	u8  message_type;
	u16 memory_flags;
	u32 operation_id;
...
struct tf_command_register_shared_memory_1 {
	u8  message_size;
	u8  message_type;
	u16 memory_flags;
	u32 operation_id;
...
struct  tf_command_register_shared_memory_schannel {
	u8  message_size;
	u8  message_type;
	u16 memory_flags;
	u32 operation_id;
	...
	u32 shared_mem_size;
	u32 shared_mem_start_offset;
	u8 shared_mem_descriptors[TF_MAX_COARSE_PAGES * 4];
};
 ...
 */
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
struct tf_command_invoke_client_command_lpae {
	u8                       message_size;
	u8                       message_type;
	u16                      param_types;
	u32                      operation_id;
 ...
 */
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
struct tf_command_management_lpae {
	u8                   message_size;
	u8                   message_type;
	u16                  command;
	u32                  operation_id;
...
union tf_command_128 {
	...
	struct tf_command_register_shared_memory_128  register_shared_memory;
...
union tf_command_8 {
	...
	struct tf_command_open_client_session       open_client_session;
	struct tf_command_close_client_session      close_client_session;
	struct tf_command_register_shared_memory_8  register_shared_memory;
	struct tf_command_release_shared_memory     release_shared_memory;
	struct tf_command_invoke_client_command     invoke_client_command;
	struct tf_command_cancel_client_operation   cancel_client_operation;
	struct tf_command_management                management;
...
union tf_command_1 {
	...
	struct tf_command_open_client_session       open_client_session;
	struct tf_command_close_client_session      close_client_session;
	struct tf_command_register_shared_memory_1  register_shared_memory;
	struct tf_command_release_shared_memory     release_shared_memory;
	struct tf_command_invoke_client_command     invoke_client_command;
	struct tf_command_cancel_client_operation   cancel_client_operation;
	struct tf_command_management                management;
...
union tf_command {
	...
	struct tf_command_open_client_session     open_client_session;
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
	struct tf_command_open_client_session_lpae open_client_session_lpae;
#endif
	struct tf_command_close_client_session    close_client_session;
	struct tf_command_register_shared_memory_schannel
	...
	struct tf_command_invoke_client_command   invoke_client_command;
#ifdef SUPPORT_LARGE_PHYSICAL_ADDRESS_EXTENSION
	struct tf_command_invoke_client_command_lpae invoke_client_command_lpae;
#endif
	struct tf_command_cancel_client_operation cancel_client_operation;
	struct tf_command_management              management;
};
...
#define IOCTL_TF_EXCHANGE        _IOWR('z', 1, union tf_command_128)
#define IOCTL_TF_EXCHANGE_8DESC        _IOWR('z', 1, union tf_command_8)
#define IOCTL_TF_EXCHANGE_1DESC        _IOWR('z', 1, union tf_command_1)
