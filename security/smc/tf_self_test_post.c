...
unsigned tf_self_test_post_vectors(void)
{
	...
	case -ENXIO:
		...
		if (tf_post_data.failures & TF_CRYPTO_ALG_INTEGRITY) {
			...
			pr_notice("[SMC Driver] integrity check initially passed\n");
		break;
	case -ENOENT:
		pr_warning("[SMC Driver] integrity check cannot be made\n");
		pr_notice("[SMC Driver] "
			  "you must pass the hmac_sha256 parameter\n");
		/* FALLTHROUGH */
	default:
		pr_err("[SMC Driver] error: SMC Driver POST FAILURE "
