...
{
	dprintk(KERN_INFO "%s: aes_state(%p) <- paes_reg(%p): CTRL=0x%08x\n",
		__func__, aes_state, paes_reg, aes_state->CTRL);
	...
	struct tf_crypto_aes_operation_state *aes_state, int encrypt)
{
	...
	dprintk(KERN_INFO "%s: paes_reg(%p) <- aes_state(%p): CTRL=0x%08x\n",
		__func__, paes_reg, aes_state, aes_state->CTRL);
...
static void tf_aes_fault_injection(u32 ctrl, u8 *buf)
{
	if (FAULTY(tf_fault_injection_mask, ctrl, ECB, 128) ||
		...
		dprintk(KERN_INFO
			"%s: no fault (mask=0x%x ctrl=0x%x mode=%u size=%u)\n",
...
static int aes_dma_init(struct aes_hwa_ctx *ctx)
{
	...
	dprintk(KERN_INFO "%s(%p) configured DMA channels (RX = %d, TX = %d)\n",
		__func__, ctx, ctx->dma_lch_in, ctx->dma_lch_out);
...
int register_smc_public_crypto_aes(void)
{
	...
	paes_reg = ioremap(AES1_REGS_HW_ADDR, SZ_1M);
	if (paes_reg == NULL)
		return -EFAULT;
