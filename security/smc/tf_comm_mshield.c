...
int __devinit tf_early_init(void)
{
	g_secure_task_id = 0;
...
#ifdef CONFIG_ARCH_OMAP5
	if (cpu_is_omap54xx()) {
		smc_l4_sec_clkdm = pdata->clkdm_lookup("l4sec_clkdm");
		hal_indexes = &omap5_hal_indexes;
	} else
#endif
#ifdef CONFIG_ARCH_OMAP4
	if (cpu_is_omap44xx()) {
		smc_l4_sec_clkdm = pdata->clkdm_lookup("l4_secure_clkdm");
...
static void tf_clock_timer_cb(unsigned long data)
{
	...
	pdata->clkdm_allow_idle(smc_l4_sec_clkdm);
...
void tf_clock_timer_start(void)
{
	...
	pdata->clkdm_wakeup(smc_l4_sec_clkdm);
	...
	u32 sdp_backing_store_addr, u32 sdp_bkext_store_addr)
{
	...
	crc = pdata->omap_secure_dispatcher(
	...
	error = pdata->omap_secure_dispatcher(
	...
	error = pdata->omap_secure_dispatcher(API_HAL_SDP_RUNTIMEINIT_INDEX,
	...
	/* RPC init */
	dpr_info("%s: RPC init...\n", __func__);
	error = pdata->omap_secure_dispatcher(API_HAL_TASK_MGR_RPCINIT_INDEX,
	...
	pdata->secure_dispatcher_replace(tf_secure_dispatcher);
	...
	u32 conf_offset, u32 conf_size)
{
	...
	pdata->sched_getaffinity(0, &saved_cpu_mask);
	ret_affinity = pdata->sched_setaffinity(0, &local_cpu_mask);
	if (ret_affinity != 0)
...
exit:
	tf_clock_timer_stop();
#ifdef CONFIG_SMP
	ret_affinity = pdata->sched_setaffinity(0, &saved_cpu_mask);
	if (ret_affinity != 0)
