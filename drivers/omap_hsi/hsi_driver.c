...
int hsi_softreset(struct hsi_dev *hsi_ctrl)
{
	...
	if (ind >= HSI_RESETDONE_MAX_RETRIES) {
		dev_err(hsi_ctrl->dev, "HSI SW_RESET failed to complete within %d retries.\n",
			HSI_RESETDONE_MAX_RETRIES);
		return -EIO;
	} else if (ind > HSI_RESETDONE_NORMAL_RETRIES) {
		dev_warn(hsi_ctrl->dev, "HSI SW_RESET abnormally long: %d retries to complete.\n",
			 ind);
	}
	...
	if (ind >= HSI_RESETDONE_MAX_RETRIES) {
		dev_err(hsi_ctrl->dev, "HSI DMA SW_RESET failed to complete within %d retries.\n",
			HSI_RESETDONE_MAX_RETRIES);
		return -EIO;
	}

	if (ind > HSI_RESETDONE_NORMAL_RETRIES) {
		dev_warn(hsi_ctrl->dev, "HSI DMA SW_RESET abnormally long: %d retries to complete.\n",
			 ind);
	}
...
static int __devinit hsi_request_mpu_irq(struct hsi_port *hsi_p)
{
	...
	if (!mpu_irq) {
		dev_err(hsi_ctrl->dev, "HSI misses info for MPU IRQ on port %d\n",
			hsi_p->port_number);
		return -ENXIO;
	}
...
static int __devinit hsi_request_cawake_irq(struct hsi_port *hsi_p)
{
	...
	if (!cawake_irq) {
		dev_err(hsi_ctrl->dev, "SSI device misses info for CAWAKE IRQ on port %d\n",
			hsi_p->port_number);
		return -ENXIO;
	}
...
static int __devinit hsi_init_gdd_chan_count(struct hsi_dev *hsi_ctrl)
{
	...
	if (!gdd_chan_count) {
		dev_warn(hsi_ctrl->dev, "HSI device has no GDD channel count (use %d as default)\n",
		...
		if (i >= 16)
			dev_err(hsi_ctrl->dev, "The Number of DMA channels shall be a power of 2! (=%d)\n",
					 ...
					 struct platform_device *pd)
{
	...
	if (!mem) {
		dev_err(&pd->dev, "HSI device does not have HSI IO memory region information\n");
		return -ENXIO;
	}
	...
	if (pdata->num_ports > HSI_MAX_PORTS) {
		dev_err(&pd->dev, "The HSI driver does not support enough ports!\n");
		return -ENXIO;
	}
...
static int __devinit hsi_platform_device_probe(struct platform_device *pd)
{
	...
	if (err < 0) {
		dev_err(&pd->dev, "Could not initialize hsi controller: %d\n",
			 err);
		goto rollback1;
	}
