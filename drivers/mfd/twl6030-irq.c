...
#if !defined(CONFIG_MACH_OMAP4_BOWSER)
/* used when battery low interrupt fires to
power off the kernel in a safe manner */
static struct work_struct twl_poweroff_work;
#endif
...
#endif
#else
	schedule_work(&twl_poweroff_work);
#endif
	return IRQ_HANDLED;
}
...
#if !defined(CONFIG_MACH_OMAP4_BOWSER)
static void twl_poweroff_func(struct work_struct *data)
{
	pr_emerg("twl_poweroff_func: kernel_power_off()\n");
	kernel_power_off();
}
#endif
...
#if !defined(CONFIG_MACH_OMAP4_BOWSER)
	INIT_WORK(&twl_poweroff_work, twl_poweroff_func);
#endif
