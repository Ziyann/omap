...
#define FW_IMAGE_NAME_EVT1  "PR1487531.img"
#define FW_IMAGE_NAME_PROTO "PR1370289_proto.img"
#define FW_IMAGE_NAME_PRE_EVT2  "PR1381609_evt2.img"
#define FW_IMAGE_NAME_LENSONE  "Soho_PR1509905_S7301B_LENS_10070001.img"
#define FW_IMAGE_NAME_TPK  "Soho_PR1509905_S7301B_TPK_11070001.img"
#define FW_IMAGE_NAME_WINTEK  "Soho_PR1509905_S7301B_WTK_12070001.img"
...
static ssize_t fwu_sysfs_sensor_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(dispconfigblockcount, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(sensorid, S_IRUGO,
			fwu_sysfs_sensor_id_show,
			synaptics_rmi4_store_error),
};
...
static int fwu_read_sensor_id(void)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			fwu->f34_fd.data_base_addr + BLOCK_DATA_OFFSET,
			snsrid_pinmux,
			sizeof(snsrid_pinmux));
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"%s: Failed to write sensor ID\n",
				__func__);
		goto reset;
	}
...
static int fwu_set_fw_image_name(void)
{
	int retval = 0;

	if (idme_get_board_revision() < 3) {
		fwu->fw_name = FW_IMAGE_NAME_PROTO;
	} else if (idme_get_board_revision() < 4) {
		fwu->fw_name = FW_IMAGE_NAME_EVT1;
	} else if (idme_get_board_revision() == 4) {
	 	fwu->fw_name = FW_IMAGE_NAME_PRE_EVT2;
	} else {
		...
		case LENSON:
			fwu->fw_name = FW_IMAGE_NAME_LENSONE;
			break;
		case TPK:
			fwu->fw_name = FW_IMAGE_NAME_TPK;
			break;
		case WINTEK:
			fwu->fw_name = FW_IMAGE_NAME_WINTEK;
			break;
		default:
			dev_err(&fwu->rmi4_data->i2c_client->dev,
				"Failed to read touch sensor id.\n");
			return -EINVAL;
		}
	}
	pr_notice("%s: Set fw_image name as - %s\n",
			 __func__, fwu->fw_name);
	return retval;
}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	deviceConfigID =  (unsigned int)config_id[3] +
	...
	dev_info(&i2c_client->dev,
			...
			fwu->config_data[2],
			fwu->config_data[3]);
	imageConfigID =  (unsigned int)fwu->config_data[3] +
			(unsigned int)fwu->config_data[2] * 0x100 +
			(unsigned int)fwu->config_data[1] * 0x10000 +
			(unsigned int)fwu->config_data[0] * 0x1000000;

	dev_info(&i2c_client->dev,
		"Device config ID %d, .img config ID %d\n",
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->fw_name == NULL) {
		retval = fwu_set_fw_image_name();
			if (retval < 0)
				return retval;
	}

	pr_notice("%s: Start of reflash process:fw_image:%s\n",
			 __func__, fwu->fw_name);
	...
	if (fwu->ext_data_source)
		...
		retval = request_firmware(&fw_entry, fwu->fw_name,
