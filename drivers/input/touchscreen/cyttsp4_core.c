...
enum cyttsp4_driver_state {
	...
	CY_OPCMD_STATE,		/* new state to allow for
...
#endif /* --CY_USE_TMA884 */
static int write_charger_hdmi_config(struct cyttsp4 *ts, u8 value);
...
static int _cyttsp4_handshake_enable(struct cyttsp4 *ts)
{
	...
	if (retval < 0) {
		dev_err(ts->dev,
			"%s: Fail read Enable Hanshake command status _cyttsp4_handshake"
			"r=%d\n", __func__, retval);
		goto _cyttsp4_set_handshake_enable_exit;
	}
...
int write_charger_hdmi_config(struct cyttsp4 *ts, u8 value)
{
	...
	if (cmd_dat[1] != 0) {
		dev_err(ts->dev,
			...
			__func__, cmd_dat[0], cmd_dat[1],
			cmd_dat[2], cmd_dat[3], cmd_dat[4]);
		retval = -EIO;
		goto cyttsp4_write_charger_hdmi_config_exit;
	}
...
cyttsp4_write_charger_hdmi_config_exit:
	...
	if (ts->suspend_blocked == true) {
		dev_vdbg(ts->dev, "%s:suspending from write_charger_hdmi\n",
			 __func__);
...
static int _cyttsp4_signal_test(struct cyttsp4 *ts)
{
	...
	printk(KERN_INFO "Row: 0x%2X , Column: 0x%2X\n", row, column);

	/* w 67 02 0B */
	cmd = 0x0B;
	retval = _cyttsp4_write_block_data(ts,
			CY_REG_BASE + offsetof(struct cyttsp4_catdata, cmd),
			sizeof(cmd), &cmd,
			ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	if (retval < 0) {
		printk(KERN_INFO "%s: Fail write command 0x%X ret= %d\n",
			__func__, cmd, retval);
		return -1;
	}
	...
	retval = _cyttsp4_read_block_data(ts,
		...
		sizeof(status), status,
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	printk(KERN_INFO "%s: status[0]:0x%X status[1]: 0x%X\n",
		__func__, status[0], status[1]);
	if (retval < 0) {
		printk(KERN_INFO "%s: Fail read command 0x%X ret= %d\n",
			__func__, cmd, retval);
		return -1;
	}
	...
	for (i = 0 ; i < ftm_test_total_points/247 + 1; ) {
		...
		cmd2[3] = 0x3C;
		cmd2[4] = 0x2;
		printk(KERN_INFO "cmd2[0]: 0x%2X , cmd2[1]: 0x%2X",
			cmd2[0], cmd2[1]);
		printk(KERN_INFO "cmd2[2]: 0x%2X , cmd2[3]: 0x%2X\n",
			cmd2[2], cmd2[3]);
		_cyttsp4_write_block_data(ts,
			CY_REG_BASE + offsetof(struct cyttsp4_catdata, data),
			sizeof(cmd2), cmd2,
			ts->platform_data->addr[CY_TCH_ADDR_OFS], true);

		_cyttsp4_read_block_data(ts,
			CY_REG_BASE + offsetof(struct cyttsp4_catdata, cmd),
			CY_NUM_CAT_DATA + 1, &cat_data.cmd,
			ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
		...
		do {
			/* delay 10 ms */
			/* According to spec, only delay 10ms,
			* actully we need 200 to 300 ms
			*/
			for (j = 0 ; j < 10; j++)
				udelay(1000);
			...
			_cyttsp4_read_block_data(ts, CY_REG_BASE +
				offsetof(struct cyttsp4_catdata, cmd),
				CY_NUM_CAT_DATA + 1, &cat_data.cmd,
				ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
			count++;
			if (count > 100) {
				printk(KERN_INFO
					"[%s] cannot get correct size of signal test\n", __func__);
				return -1;
			}
...
static void _cyttsp4_file_free(struct cyttsp4 *ts)
{
	...
	device_remove_file(ts->dev, &dev_attr_ic_ver_raw);
#if defined(CY_USE_FORCE_LOAD) || defined(CONFIG_TOUCHSCREEN_DEBUG)
...
#endif
#ifdef CONFIG_MACH_OMAP4_BOWSER_SUBTYPE_JEM_FTM
	device_remove_file(ts->dev, &dev_attr_ftm_test);
#endif
#ifdef CONFIG_TOUCHSCREEN_DEBUG_ENABLE_ENTRY
	device_remove_file(ts->dev, &dev_attr_ts_debug);
#endif
}
...
static irqreturn_t cyttsp4_irq(int irq, void *handle)
{
	...
	case CY_OPCMD_STATE:
	{
		...
		if (retval < 0) {
			dev_err(ts->dev,
				"%s: unable to read cmd_status in ISR\n",
				__func__);
...
#endif
	/* delete this line, if keybit is set to KEY_MAX ,		*/
