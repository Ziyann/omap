...
struct bq27541_info {
	...
	int suspend_drain;
	int suspend_drain_high; /* _high == high-precision */
...
#if defined(CONFIG_AMAZON_METRICS_LOG)
static int bq27541_get_capacity_high(struct bq27541_info *info)
{
	int value;
	int err = 0;
	...
	dev_info(&bq27541_battery_i2c_client->dev,
		"%s: info->battery_remaining_charge %d, info->battery_full_charge %d\n",
		__FUNCTION__, info->battery_remaining_charge, info->battery_full_charge);
	...
	if (err) {
		...
		return info->battery_remaining_charge * 10000 /
...
battery_metrics_locked(struct bq27541_info *info)
{
	...
	if ((info->flags & BQ27541_STATUS_VALID) &&
		...
		/* suspend_capacity* deltas will be negative when
		 * charging on AC (not USB), and * also in the case of
		 * an SOC jump, which on the bq27541 may be large.
		 */
		if (info->battery_status == POWER_SUPPLY_STATUS_CHARGING)
			return;

                /*
                 * Whole-second quantization: counter will wrap in ~25000 days.
                 */
		info->suspend_realtime += diff.tv_sec;

		if (info->suspend_capacity >= 0) {
			info->suspend_drain += info->suspend_capacity - info->battery_capacity;
		} else {
			dev_warn(&bq27541_battery_i2c_client->dev,
				"invalid suspend_capacity, "
				"sysfs suspend_drain not updated\n");
		}

		if (info->suspend_capacity_high >= 0) {
			int capacity_high = bq27541_get_capacity_high(info);
			int capacity_diff = info->suspend_capacity_high - capacity_high;

			dev_info(&bq27541_battery_i2c_client->dev,
				"resuming, now-stale info->battery_status %s\n",
				info->battery_status);
			dev_info(&bq27541_battery_i2c_client->dev,
				"resuming,  info->suspend_capacity_high %d, "
				"capacity_high %d\n",
				info->suspend_capacity_high, capacity_high);

			info->suspend_drain_high += capacity_diff;
		} else {
			dev_warn(&bq27541_battery_i2c_client->dev,
				"invalid suspend_capacity_high, "
				"sysfs suspend_drain_high not updated\n");
		}
	}
}
#endif
...
static void battery_handle_work(struct work_struct *work)
{
	...
	if (err) {
		...
		if (value == 100 && info->battery_capacity != value) {
			char buf[128];
			snprintf(buf, sizeof(buf),
				"bq27541:def:POWER_STATUS_FULL_CHARGE=1;CT;1,"
				"cap=%u;CT;1:NR", 100);
			log_to_metrics(ANDROID_LOG_INFO, "battery", buf);
		}
#endif
...
static int bq27541_battery_suspend(struct i2c_client *client, pm_message_t state)
{
	...
	/* Cache suspend capacity, high-precision */
	info->suspend_capacity_high = bq27541_get_capacity_high(info);
