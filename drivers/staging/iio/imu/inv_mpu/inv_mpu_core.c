...
static int inv_do_power_up_locked(struct inv_mpu_iio_s *st)
{
	int err = 0;

	if (!err)
		err = inv_switch_engine(st, st->chip_config.gyro_enable,
				BIT_PWR_GYRO_STBY);
	if (!err)
		err = inv_switch_engine(st, st->chip_config.accl_enable,
				BIT_PWR_ACCL_STBY);
	if (!err)
		err = inv_lpa_freq(st, st->chip_config.lpa_freq);

	if (!err)
		st->chip_config.is_asleep = false;
	return err;
}
...
static int set_power_itg(struct inv_mpu_iio_s *st, bool power_on)
{
	...
	struct inv_reg_map_s *reg = &st->reg;
	u8 data;
	int err;
	...
	err = inv_i2c_single_write(st, reg->pwr_mgmt_1, data);
	if (power_on) {
		if (!(st->flags & F_INV_RESUMING)) {
			if (!err) {
				msleep(POWER_UP_TIME);
				err = inv_do_power_up_locked(st);
			}
		} else {
			/* ignore error, because we will retry, if necessary */
			err = 0;
			schedule_delayed_work(&st->inv_work,
				msecs_to_jiffies(POWER_UP_TIME));
		}
	} else {
		if (!err)
...
static void inv_process_work(struct work_struct *work)
{
	struct inv_mpu_iio_s *st = container_of(work,
			struct inv_mpu_iio_s, inv_work.work);
	bool retry = false;
	mutex_lock(&st->power_lock);
	if (st->flags & F_INV_RESUMING) {
		int err;
	...
	struct device_attribute *attr, char *buf)
{
	struct inv_mpu_iio_s *st = iio_priv(dev_get_drvdata(dev));
	int orient;
	spin_lock_irq(&st->config_lock);
	orient = st->display_orient_data;
	spin_unlock_irq(&st->config_lock);
	return sprintf(buf, "%d\n", orient);
}
	...
	const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&st->inv_work, inv_process_work);
...
static void inv_mpu_shutdown(struct i2c_client *client)
{
	...
	/* disable irq handling from this point onwards */
	if (st->client->irq)
...
static int inv_mpu_suspend(struct device *dev)
{
	...
	cancel_delayed_work_sync(&st->inv_work);
