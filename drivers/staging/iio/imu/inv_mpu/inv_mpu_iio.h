 ...
 *  @power_lock:    Protects power sequence and power state
 *  @time_stamp_lock: Protects access to time stamp (hard IRQ and thread IRQ)
 *  @inv_work:      delayed processing at resume time
 *  @timestamps:    kfifo queue to store time stamp.
 *  flags:          describe type of delayed processing required
 *  @trig;              iio trigger.
...
struct inv_mpu_iio_s {
	...
	struct delayed_work inv_work;
#define TIMESTAMP_FIFO_SIZE 16
	DECLARE_KFIFO(timestamps, u64, TIMESTAMP_FIFO_SIZE);
#define F_INV_RESUMING		0x01
	u32	flags;
