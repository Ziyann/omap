					...
					u8 *data, s64 t)
{
	...
	if (conf->dmp_on && (conf->tap_on || conf->display_orient_on)) {
		...
		bool display_orient_data_changed = false;
		unsigned long flags;
		word = (u32)(be32_to_cpup((u32 *)&data[ind]));
		source = ((word >> 16) & 0xff);

		spin_lock_irqsave(&st->config_lock, flags);
		st->tap_data = (DMP_MASK_TAP & word);
		display_orient_data =
		((DMP_MASK_DIS_ORIEN & word) >> DMP_DIS_ORIEN_SHIFT);
		if (display_orient_data != st->display_orient_data) {
			display_orient_data_changed = true;
			st->display_orient_data = display_orient_data;
		}
		spin_unlock_irqrestore(&st->config_lock, flags);

		/* report tap information */
		if (conf->tap_on && (source & INT_SRC_TAP))
			sysfs_notify(&indio_dev->dev.kobj, NULL, "event_tap");
		/* report orientation information */
		if (conf->display_orient_on &&
			display_orient_data_changed)
