...
int mmc_gen_cmd(struct mmc_card *card, void *buf, u8 index, u8 arg1, u8 arg2, u8 mode)
{
	...
	struct scatterlist sg;
	tsb_cmd_format *tsb_cmd;
	void *data_buf;
	static u8 smartpwd_cmd[8] = { 0x07, 0x00, 0x00, 0x00, 0x26, 0xE9, 0x01, 0xEB };
	...
	if (mode == 0) {
		...
		tsb_cmd = (void *) data_buf;
//		pr_info("TSB CMD:0x%x : PWD:0x%x%x%x%x\n", tsb_cmd->sub_cmd_no,
//			tsb_cmd->pwd1,
//			tsb_cmd->pwd2,
//			tsb_cmd->pwd3,
//			tsb_cmd->pwd4);
...
//	pr_info("%s: TSB smart cmd execute successfully\n", __func__);
	return 0;
}
...
int mmc_toshiba_report(struct mmc_card *card, u8 *buf)
{
	...
	if (!err) {
		...
		hbblk = (void*) buf;
		pr_info("eMMC Health CMD:0x%x sts:0x%x, mlc(max:%d avg:%d), slc(max:%d avg:%d)\n",
		hbblk->sub_cmd_no, hbblk->status,
		__swap32gen(hbblk->mlc_write_erase_max), __swap32gen(hbblk->mlc_write_erase_avg),
		__swap32gen(hbblk->slc_write_erase_max), __swap32gen(hbblk->slc_write_erase_avg));
	}
