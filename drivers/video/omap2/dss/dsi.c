...
static int dsi_cio_init(struct omap_dss_device *dssdev)
{
	...
	r = dss_dsi_enable_pads(dsi_get_dsidev_id(dsidev), 0);
	if (r)
		return r;
...
err_scp_clk_dom:
	dsi_disable_scp_clk(dsidev);
	dss_dsi_disable_pads(dsi_get_dsidev_id(dsidev), 0);
	return r;
}
...
#if defined(CONFIG_PANEL_NT71391_HYDIS) || defined(CONFIG_PANEL_S6E8AA0)
		u8 data_cmd = 0;
		...
		u8 *buf, int buflen, enum dss_dsi_content_type type)
{
	...
	if (dt == MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT) {
		...
		return 2;
	} else if (dt == MIPI_DSI_RX_GENERIC_LONG_READ_RESPONSE ||
				 dt == MIPI_DSI_RX_DCS_LONG_READ_RESPONSE) {
		int w;
...
static int dsi_display_init_dispc(struct omap_dss_device *dssdev)
{
	...
	if (dssdev->panel.dsi_mode == OMAP_DSS_DSI_CMD_MODE) {
		...
		dispc_set_dithering(dssdev->manager->id, dssdev->ctrl.dither);
	return 0;
}
