...
#ifdef CONFIG_HAS_EARLYSUSPEND
	max97236->early_suspended = false;
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static int max97236_early_suspend(struct early_suspend *h)
{
	struct max97236_priv *max97236 = container_of(h, struct max97236_priv,
			early_suspend);

	max97236->early_suspended = true;

	return 0;
}
static int max97236_late_resume(struct early_suspend *h)
{
	struct max97236_priv *max97236 = container_of(h, struct max97236_priv,
			early_suspend);

	max97236_resume(max97236->codec);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	max97236->early_suspend.suspend = max97236_early_suspend;
	max97236->early_suspend.resume = max97236_late_resume;
	max97236->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	register_early_suspend(&max97236->early_suspend);
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&max97236->early_suspend);
#endif
	wake_lock_destroy(&max97236->wakelock);
